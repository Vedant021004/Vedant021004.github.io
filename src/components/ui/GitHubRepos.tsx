...
import { Code2, Star, GitFork, ArrowUpRight, Globe } from "lucide-react";
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {repos.map((repo, index) => {
            const links = (dataJson.projectLinks || {}) as Record<string, string>;
            const matchedKey = Object.keys(links).find(
              k => k.toLowerCase().trim() === repo.name.toLowerCase().trim()
            );
            ...
            return (
              <motion.div
                ...
                className="group flex flex-col justify-between p-8 bg-white dark:bg-[#111111] border-2 border-black/10 dark:border-white/10 rounded-3xl hover:border-black dark:hover:border-white transition-colors"
              >
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="text-xl md:text-2xl font-bold text-black dark:text-white group-hover:text-[#0d99ff] transition-colors line-clamp-1">
                      {repo.name}
                    </h3>
                    <a 
                      href={repo.html_url} 
                  ...
                  <p className="text-gray-600 dark:text-gray-400 mb-6 line-clamp-2 h-12 transition-colors">
                ...
                <div className="flex flex-col gap-4 mt-auto">
                  {demoUrl && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center justify-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-full transition-colors w-fit shadow-sm"
                    >
                      <Globe className="w-3.5 h-3.5" /> Live Demo
                    </a>
                  )}

                  <div className="flex items-center justify-between text-sm font-semibold transition-colors pt-2 border-t border-black/5 dark:border-white/5">
                    {/* Language Pill */}
